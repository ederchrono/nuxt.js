...
export default class Renderer extends Tapable {
  constructor (nuxt) {
    ...
    console.log('inside renderer', this.options)
  ...
  async renderRoute (url, context = {}) {
    ...
    if (this.noSSR || spa) {
      ...
      console.log('App setting id!!!')
      console.log(this.options.generate)
      const appId = this.options.generate.appId || '__nuxt'
      const APP = `<div id="${appId}">${this.resources.loadingHTML}</div>`
    ...
    if (!context.nuxt.serverRendered) {
      console.log('App setting id SR!!!')
      console.log(this.options.generate)
      const appId = this.options.generate.appId || '__nuxt'
      APP = `<div id="${appId}"></div>`
    }
    ...
    const windowScript = this.options.generate.windowScript || '__NUXT__'
    APP += `<script type="text/javascript">window.${windowScript}=${serialize(context.nuxt, { isJSON: true })};</script>`
  ...
  async renderAndGetWindow (url, opts = {}) {
    ...
    const windowScript = this.options.generate.windowScript || '__NUXT__'
    const nuxtExists = window.document.body.innerHTML.includes(`window.${windowScript}`)
    /* istanbul ignore if */
    if (!nuxtExists) {
